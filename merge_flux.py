...
def clean_prompt(text):
    """清理 prompt 文本"""
    text = re.sub(r'\d+\s*(Stable Diffusion|FLUX|Flux)$', '', text)
...
# 读取 Flux 数据
flux_data = json.loads(Path('/tmp/flux_raw.json').read_text())
print(f"Flux 抓取: {len(flux_data)} 条")
